...
namespace AWork.Persistence.Repositories.Sales
{
    public class SalesPersonRepository : RepositoryBase<SalesPerson>, ISalesPersonRepository
    {
        ...
        public async Task<IEnumerable<PersonEmployeeData>> GetDataEmployeeTypeSP(string name)
        {
            var businessEntity = new SqlParameter("@paramId", name);
            var rawSQL = await _dbContext.PersonEmployeeDataSQL
                ...
                "where p.FirstName = @paramId " +
                "group by p.BusinessEntityID, (p.FirstName + ' ' + p.LastName), e.JobTitle, e.NationalIDNumber ", businessEntity)
        ...
        public async Task<IEnumerable<GetDataTerritory>> GetDataTerritoryByName(string name)
        {
            var territory = new SqlParameter("@Id", name);
            var rawSQL = await _dbContext.DataTerritorySQL
                .FromSqlRaw("select * from Sales.SalesTerritory " +
                "where Name = @Id ", territory)
